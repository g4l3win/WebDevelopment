...
<?PHP 
  ...
  if(isset($_GET['kategori'])){
    $pilihkategori = urldecode($_GET['kategori']);
    //query untuk dapat data yang di select
    $query = "select * from destinasi_a d JOIN kategoriwisata_a a
    ON d.kategoriKODE = a.kategorikode where a.kategorikode ='$pilihkategori'";
...
<body>
  ...
  <?php if(mysqli_num_rows($pilihdata)>0) {
              ...
              {?>
      <div class="card text-dark bg-info col-mb-11" >
        <div class="row g-0">
          <div class="col-md-5">
            <div class="card-body">
              <h5 class="card-title">Kategori <?php echo $row["kategorinama"];?> <small class="text-muted" style="color:lightgray;"><?php echo $row["destinasiNAMA"];?></small></h5>
