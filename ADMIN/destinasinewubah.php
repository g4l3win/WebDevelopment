            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid px-4">
                        <h1 class="mt-4">Gambar Destinasi</h1>
                        <ol class="breadcrumb mb-4">
                            <li class="breadcrumb-item active">Kelola gambar destinasi</li>
                        </ol>
<html lang="en">
<head>
	<meta charset="UTF-8">
...
<?php 
	...
	if(isset($_POST['ubah']))
	{
		...
		if(($ekstensifile == "jpg") or ($ekstensifile == "JPG") or ($ekstensifile == "png"))
		{
			...
			mysqli_query($connection, "update destinasi_a set destinasiNAMA='$namafoto', 
			destinasiKET = '$keterangandestinasi', destinasiFOTO = '$namafile', 
			kategoriKODE = '$kodekategori'
			WHERE destinasiKODE = '$kodefoto'");
			header("location:destinasinew.php");
		}

	}

	$datakategori = mysqli_query($connection, "select * from kategoriwisata_a");
	$destinasikode = $_GET['ubafoto'];
	$editdestinasi = mysqli_query($connection, "select * from destinasi_a where destinasiKODE = '$destinasikode'");
	$rowedit = mysqli_fetch_array($editdestinasi);

	$editkategori = mysqli_query($connection, "select * from destinasi_a d JOIN kategoriwisata_a k
	ON d.kategoriKODE = k.kategorikode
	...
	<table class="table table-hover table-dark">
		...
		<tbody>
			...
			if (isset($_POST["kirim"])){
                ...
                $query=mysqli_query($connection,"select * from destinasi_a 
			...
			{ ?>
				<tr>
					...
					<td>
						<a href="destinasinewubah.php?ubafoto=<?php echo $row['destinasiKODE']?>" class="btn btn-success btn-sm" title="EDIT">
					...
					<td>
						<a href="destinasinewhapus.php?hapusfoto=<?php echo $row['destinasiKODE']?>" class="btn btn-danger btn-sm" title="DELETE">
