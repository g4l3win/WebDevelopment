            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid px-4">
                        <h1 class="mt-4">Kelola data winni setiawati</h1>
                        <ol class="breadcrumb mb-4">
                            <li class="breadcrumb-item active">Tambah, hapus, edit data Winni Setiawati</li>
                        </ol>
<?php 
...
if(isset($_POST['edit'])){
    ...
    $winniID = $_POST["winniID"];
    $winniKOTA = $_POST["winniKOTA"];
    $destinasiKODE = $_POST["kodedestinasi"];

    //SIMPAN DI TABEL KATEGORI WISATA
    $result=mysqli_query($connection, "update winnisetiawati set winniKOTA='$winniKOTA',
    destinasiKODE = '$destinasiKODE'
    where winniID = '$winniID'");
    
    if (!$result) {
        die('Query Error: ' . mysqli_error($connection));
    } else {
        header("location:winnisetiawati.php");
    }
}
    $datadestinasi = mysqli_query($connection, "select * from destinasi_a");
    $winniID = $_GET['ubah'];
    $editwinnisetiawati = mysqli_query ($connection, "select * from winnisetiawati 
    where winniID = '$winniID'");
    $rowedit=mysqli_fetch_array($editwinnisetiawati);
    $editdestinasi =  mysqli_query($connection, "select * from destinasi_a d join winnisetiawati s
    ON d.destinasiKODE = s.destinasiKODE
    where winniID = '$winniID'");
...
<body>
    ...
    <form method="POST">
                ...
                <div class="mb-3 row">
                    <label for="winniID" class="col-sm-2 col-form-label">Kode Winni Setiawati </label>
                        <div class="col-sm-10">
                        <input type="text" class="form-control" name="winniID" id="winniID" maxlength="4" placeholder="input kode Winni"
                        value="<?php echo $rowedit["winniID"];?>" readonly>
                    </div>
                </div>
                <div class="mb-3 row">
                    <label for="winniKOTA" class="col-sm-2 col-form-label">Winni Kota </label>
                        <div class="col-sm-10">
                        <input type="text" class="form-control" name="winniKOTA" id="winniKOTA" placeholder="input WINNI KOTA"
                        value="<?php echo $rowedit["winniKOTA"];?>">
                    </div>
                </div>
                        ...
                        <select name="kodedestinasi" class="form-control" id="kodedestinasi">
                            ...
                            <?php 
                                ...
                                <option value="<?php echo $row['destinasiKODE'];?>">
                                    <?php echo $row['destinasiKODE'];?> <!--Pakai nama kolom di tabel database-->
            ...
            <div class="jumbotron jumbotron-fluid mb-5 row">
                <div class="container">
                    <h1 class="display-4">Daftar Winni </h1>
            </div>
            </div>
                ...
                <form method = "POST"> 
                    <div class= "form-group row mb-2">
                        ...
                        <div class="col-sm-6">
                            ...
                            placeholder="cari nama kota winni">
                        </div>
            ...
            <table class="table table-dark table-hover">
                <thead>
                    <tr>
                    <th scope="col">No</th>
                    <th scope="col">Kode winni</th>
                    <th scope="col">Nama kota winni</th>
            ...
            <?php 
                ...
                if (isset ($_POST["kirim"])){
                    $search = $_POST['search'];
                    $query = mysqli_query ($connection, "select winnisetiawati.* 
                    from winnisetiawati where winniKOTA LIKE '%".$search."%'");
                }else {
                    //kalau enggak di search jalanin yang else
                    $query = mysqli_query($connection, "select winnisetiawati.* from winnisetiawati
            ...
            ?>
                ...
                INI BAKAL DIPHP-in buat munculin yang ketemu pas searching-->
                        ...
                        <td><?php echo $row['winniID'];?></td>
                        <td><?php echo $row['winniKOTA'];?></td>
                        <td><?php echo $row['destinasiKODE'];?></td>
                        <td width="5px">
                            <a href="winnisetiawatiedit.php?ubah=<?php echo $row["winniID"]?>" class="btn btn-success btn-sm" title="EDIT">
                        ...
                        <td width="5px">
                            ...
                            <a href="winnisetiawatihapus.php?hapus=<?php echo $row["winniID"]?>" class="btn btn-danger btn-sm" title="HAPUS">
            ...
            <?php }?>  <!-- INI PUNYA PHP JADI HARUS DIAPIT PHP     -->
                ...
                <!--------------------- PAGING -- MEMBUAT GANTI HALAMAN ---------------->
                    <?php 
                        $query = mysqli_query($connection, "SELECT * FROM souvenir");
