...
<?php 
	...
	if(isset($_POST['ubah']))
	{
		...
		if(($ekstensifile == "jpg") or ($ekstensifile == "JPG") or ($ekstensifile == "png"))
		{
			...
			mysqli_query($connection, "update travel set fotodestinasiNAMA= '$namafoto', 
			fotodestinasiFILE ='$namafile', 
			destinasiKODE= '$kodedestinasi', travelKODE ='$kodetravel'
	...
	$datafoto = mysqli_query($connection, "select * from travel");
	$fotokode = $_GET['ubafoto'];
	$editfoto = mysqli_query($connection,"select * from travel where fotodestinasiKODE = '$fotokode'");
	...
	$editdestinasi = mysqli_query($connection, "select * from travel f JOIN destinasi_a d
	ON f.destinasiKODE = d.destinasiKODE
    where fotodestinasiKODE = '$fotokode'");
    $rowedit2 = mysqli_fetch_array($editdestinasi);

	$edittravel = mysqli_query($connection, "select * from travel f join detailtravel d
	on f.travelKODE = d.travelKODE 
	where fotodestinasiKODE = '$fotokode'");
	$rowedit3 = mysqli_fetch_array($edittravel);
